...
valid_input:
    ;----------------------------------------
    ; Подблок: Инициализация начальных значений
    ;----------------------------------------
    mov ecx, [num_terms]   ; Загружаем счетчик цикла (число членов)
    mov dword [sum], 0     ; Инициализируем сумму нулем
    mov esi, 1             ; k начинается с 1
    
    ;----------------------------------------
    ; Подблок: Цикл вычисления суммы
    ;----------------------------------------
calculate_sum:
    ; Проверяем, не закончились ли итерации
    cmp ecx, 0             ; Сравниваем счетчик с 0
    jle calculation_done   ; Если меньше или равно 0, завершаем вычисления
    
    ; Вычисление текущего члена ряда Ak = 3*k-1
    mov eax, 3             ; Загружаем 3 в eax
    mul esi                ; eax = 3*esi (3*k)
    sub eax, 1             ; eax = 3*k-1
    
    ; Сохраняем промежуточные результаты перед вызовом функции
    mov ebx, eax           ; Сохраняем значение члена ряда в ebx
    mov edi, ecx           ; Сохраняем счетчик цикла в edi
    
    ;----------------------------------------
    ; Подблок: Вывод текущего члена ряда
    ;----------------------------------------
    push ebx               ; Значение члена ряда (3*k-1)
    push esi               ; Номер члена (k)
    push term_fmt          ; Формат вывода
    ...
    add [sum], eax         ; sum += (3*k-1)
    
    ; Увеличиваем k и уменьшаем счетчик
    inc esi                ; k++
    dec ecx                ; Уменьшаем счетчик итераций
    
    ; Проверяем счетчик и продолжаем цикл
    jnz calculate_sum      ; Если счетчик не 0, повторяем цикл
